...
func getResultForStepMatrix(result model.Value, r *KeptnPrometheusProvider)(map[time.Time]float64, error) {
	// check if we can cast the result to a matrix
	resultMatrix, ok := result.(model.Matrix)
	if !ok {
		return nil, fmt.Errorf("could not cast result")
	...
	resultMap := make(map[time.Time]float64)

	for i:=0; i < len(resultMatrix[0].Values); i++ {
		resultMap[resultMatrix[0].Values[i].Timestamp] = resultMatrix[0].Values[i].Value.String()
	}
